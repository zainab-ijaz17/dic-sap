...
function LabelPrintingPage({ user, onLogout }) {
  ...
  const fetchLabelData = async (batchValue) => {
    ...
    try {
      ...
      const [{ quantity, uom }, expirationDate, bin] = await Promise.all([
        ...
        <div style={{ background: "white", borderRadius: "12px", padding: "1.5rem", boxShadow: "0 4px 12px rgba(0,0,0,0.06)" }}>
          ...
          {label && (
            <div style={{ border: "2px dashed #9ca3af", borderRadius: "10px", padding: "1.25rem", background: "#fafafa" }}>
              <div style={{ marginBottom: "0.5rem" }}><strong>Material Number:</strong> {label.materialNumber}</div>
              <div style={{ marginBottom: "0.5rem" }}><strong>Description:</strong> {label.materialDescription}</div>
              <div style={{ margin: "0.75rem 0", textAlign: "center" }}>
                <BarcodeDisplay value={label.batch} />
              </div>
              <div style={{ marginBottom: "0.5rem" }}><strong>Quantity:</strong> {label.quantity} {label.uom}</div>
